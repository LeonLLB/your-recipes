...
const PaginationBarItem = ({number,onClick}:{number:number,onClick:()=>void}) => {
  ...
  return <div className={`py-1 px-2 sm:py-2 sm:px-4 text-lg rounded-md border border-teal-400 ${(pageNum === number || (router.asPath === '/' && number === 1)) ? 'bg-green-500' : 'bg-emerald-50'}`}>
